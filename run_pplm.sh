...
files=("legal" "politics" "religion" "science" "technologies")

for file in "${files[@]}"
do
    for str in "${strings[@]}"
    do
        python run_pplm.py -B $file --cond_text "$str" --length 50 --gamma 1.5 --num_iterations 3 --num_samples 1 --stepsize 0.03 --window_length 5 --kl_scale 0.01 --gm_scale 0.99 --colorama --sample --verbosity "quiet" >> "${file}_output.txt"
    done
done
